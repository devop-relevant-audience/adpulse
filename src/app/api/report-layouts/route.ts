...
import { getMasterReportTemplate, getReportTemplate } from "@/lib/data/report-templates";
...
// A new layout starts blank, from another layout of the same client, from an
// agency report template, or from the master — never two of those at once.
const postSchema = z
  .object({
    clientId: z.string().uuid(),
    name: z.string().min(1).max(120),
...
export const POST = withRoute("report-layouts.POST", async (request: NextRequest) => {
  ...
  if (parsed.data.duplicateFromId) {
    ...
    // The master is created from the built-in report preset if it doesn't exist
    // yet, so this branch always has content to stamp.
    from = await getMasterReportTemplate();
  }
