...
import { BiRefresh, BiLockAlt, BiFile, BiMessageRounded } from "react-icons/bi";
...
interface ParsedReport {
  ...
  /** Set for a view report: the frozen grid replaces the classic ReportViewer. */
...
function parseReportResponse(raw: Record<string, unknown>): ParsedReport {
  ...
  return {
    ...
    snapshot: isViewSnapshot(raw.viewSnapshot) ? raw.viewSnapshot : null,
  };
}
      ...
      <div className="flex h-screen overflow-hidden bg-[#f8f7f6]">
        ...
        >
          <header className="bg-white border-b border-hairline sticky top-0 z-10 shrink-0">
            {/* Match the body's container so the header lines up with the grid. */}
            <div
              className={cn(
                "mx-auto px-6 py-4 flex items-center justify-between",
                snapshot ? "max-w-7xl" : "max-w-4xl"
              )}
            >
              <div className="flex items-center gap-2.5">
                <LogoMark size={32} />
                <div>
                  {/* `clientName` is derived from a classic report's title,
                      which a view report does not follow — name its view
                      instead. A DOCUMENT report is different again: its
                      `viewName` is the internal report layout's, so it shows
                      the report's own title (what `clientName` holds here). */}
                  <p className="text-sm font-semibold text-ink">
                    {snapshot && snapshot.pageStyle !== "document"
                      ? snapshot.viewName
              ...
              <div className="flex items-center gap-2">
                ...
                <span className="text-[12px] text-ink-muted font-medium">Shared report</span>
              </div>
            </div>
          </header>
          <main className="flex-1 overflow-y-auto">
            {/* A view report is a 12-column dashboard grid, so it gets the wider
                container; classic reports keep their original max-w-4xl page. */}
            <div className={cn("mx-auto px-6 py-8", snapshot ? "max-w-7xl" : "max-w-4xl")}>
              {snapshot ? (
      ...
      <div className="bg-white rounded-2xl border border-hairline shadow-sm p-8 max-w-sm w-full">
        <div className="flex flex-col items-center text-center mb-6">
          <div className="w-12 h-12 rounded-xl bg-primary/10 flex items-center justify-center mb-3">
            <BiFile className="w-6 h-6 text-primary" />
          </div>
        ...
        <div className="space-y-3">
          <div className="relative">
            <label htmlFor="report-password" className="sr-only">Report password</label>
            <BiLockAlt className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-ink-muted" />
            <Input
              id="report-password"
              type="password"
